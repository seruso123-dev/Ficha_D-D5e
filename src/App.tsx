...
export default function App() {
  ...
  const resetCharacter = () => {
    // We avoid window.confirm as it may not work in the iframe environment
    // Use deep clone to ensure nested objects are fresh
    setChar(JSON.parse(JSON.stringify(INITIAL_DATA)));
    setProfBonus(2);
    setActiveTab('combat');
    setSpellSearch('');
    setSpellFilter({
